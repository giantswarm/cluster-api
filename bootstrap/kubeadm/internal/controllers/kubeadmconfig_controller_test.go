...
func TestBootstrapTokenRefreshIfTokenSecretCleaned(t *testing.T) {
	t.Run("should not recreate the token for Machines", func(t *testing.T) {
		...
		k := &KubeadmConfigReconciler{
			...
			KubeadmInitLock:     &myInitLocker{},
			TokenTTL:            DefaultTokenTTL,
			Tracker:             remote.NewTestClusterCacheTracker(logr.New(log.NullLogSink{}), myclient, remoteClient, myclient.Scheme(), client.ObjectKey{Name: cluster.Name, Namespace: cluster.Namespace}),
		}
		request := ctrl.Request{
			NamespacedName: client.ObjectKey{
				Namespace: metav1.NamespaceDefault,
